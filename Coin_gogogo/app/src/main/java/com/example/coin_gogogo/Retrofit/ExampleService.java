...
public interface ExampleService {
    ...
    Single<Transaction_List_Response> TRANSACTION_LIST_SINGLE(
            @Path("path") String path,
            @Query("count") Integer count); // ?count={count}를 나타냄 -> 원하는 리스트 갯수요청

    @GET("public/transaction_history/{path}_KRW")
    Observable<Transaction_List_Response> TRANSACTION_LIST_OBS(
            @Path("path") String path,
            @Query("count") Integer count); // ?count={count}를 나타냄 -> 원하는 리스트 갯수요청
    ...
    @GET("public/ticker/{path}_KRW")
    Observable<Ticker_Response> TICKER_OBS(
            @Path("path") String path
    );
